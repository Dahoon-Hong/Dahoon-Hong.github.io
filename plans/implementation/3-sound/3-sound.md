# 3. 사운드 라이선스 및 상업 이용 안전 지침

## 현재 적용 계획

이 단계는 사운드 런타임을 먼저 만들기 위한 단계가 아니라, 3.1단계 효과음과 3.2단계 배경음악이 상업 배포에 사용할 수 있는 음원만 소비하도록 기준과 증빙 절차를 고정하는 단계다. 모든 사운드 asset은 이 문서의 승인 규칙과 provenance 기록을 통과한 뒤에만 시스템에 연결한다.
...
- [3.1-sound-effects.md](3.1-sound-effects.md)
- [3.2-background-music.md](3.2-background-music.md)
- [100-release-verification.md](../100-release-verification/100-release-verification.md)
...
- 3.1단계와 3.2단계가 승인되지 않은 음원을 시스템에 연결할 수 없도록 한다.
...
| 출처 또는 라이선스 | 상업 이용 판단 | 3단계 기본 상태 | 조건 |
| --- | --- | --- | --- |
...
따라서 3.1·3.2단계의 기본 음원은 Kenney CC0 또는 직접 제작 음원으로 구성한다. Freesound/OpenGameArt는 대체 음원이 없고 파일별 검토 기록을 남길 때만 예외적으로 사용한다.

## GPT와 AI 생성 오디오 정책

OpenAI 공식 문서에서 확인되는 Audio API의 안정적인 파일 생성 기능은 text-to-speech다. 이 기능은 대사나 내레이션을 만들 수 있지만, 이번 범위에는 대사를 포함하지 않는다. 공식 문서에서 일반적인 게임 효과음·배경음악 생성 권한을 별도로 확인하지 못했으므로 3.1·3.2단계의 release asset으로 사용하지 않는다.
...
- API TTS를 추후 사용하려면 적용되는 API 약관, voice consent, AI disclosure, 출력 생성 일시와 모델 정보를 별도 기록하고 3단계 예외 승인을 다시 받는다.

## Provenance 및 manifest 계약

3단계 구현 시 다음 문서와 데이터 계약을 만든다.
...
- 필요한 event를 3.1단계와 3.2단계 ID 목록으로 먼저 고정한다.
...
- 3.1단계와 3.2단계는 src/data/audio.json에 등록된 approved ID만 AudioManager에 넘긴다.
- asset 제작만 완료하고 Game 또는 HUD event에 연결하지 않은 상태를 완료로 인정하지 않는다.
- audio manifest 검증 명령을 build 전에 실행한다.
- 3.1단계는 sfx bus, 3.2단계는 music bus를 사용하며 두 bus 모두 3단계 provenance ID를 역참조할 수 있어야 한다.
...
- [ ] 3.1단계가 사용할 SFX ID와 3.2단계가 사용할 music ID가 구분되었다.
- [ ] 3.1·3.2단계가 승인되지 않은 음원을 runtime에 연결하지 않는다는 인계 조건이 남았다.
