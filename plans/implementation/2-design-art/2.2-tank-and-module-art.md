# 2.2. 탱크·코어·전투 모듈 asset 상세 계획

## 현재 적용 계획

2.1단계의 공통 asset 계약을 기준으로 starter 탱크와 모듈 시각 asset을 제작한다. 이 단계에서는 플레이어 탱크의 구성과 격자 설치 상태를 명확하게 만드는 데 집중하고, 실제 Canvas renderer 연결은 2.6단계에서 수행한다.

선행 계획:

- [2.1-art-direction-and-asset-contract.md](2.1-art-direction-and-asset-contract.md)
- [1.7-mid-term-review.md](../1-core-gameplay/1.7-mid-term-review.md)
- [1.8-progression-content.md](../1-core-gameplay/1.8-progression-content.md)

후속 계획:

- [2.3-enemy-art.md](2.3-enemy-art.md): 같은 팔레트와 외곽선 규칙으로 적을 제작한다.
- [2.4-resource-projectile-effects-art.md](2.4-resource-projectile-effects-art.md): 무기 발사체와 피격 효과가 모듈 실루엣을 보완한다.
- [2.6-asset-loader-canvas-integration.md](2.6-asset-loader-canvas-integration.md): 이 단계의 asset과 manifest를 실제 게임에 연결한다.
- [2.7-hud-upgrade-ui-art.md](2.7-hud-upgrade-ui-art.md): 모듈 world asset과 대응하는 UI icon을 사용한다.
...
4. `machine-gun-12.7mm`과 `mortar-60mm`의 차이
...
| 초기 전투 배치 | `machine-gun-12.7mm` at `{ x: 1, y: 0 }` | 첫 화면에 Gatling Cannon 하나만 기본 설치한다. |
...
| `tank.module.machine-gun-12.7mm` | `active`, `damaged`, `disabled` | `44x44` | 발사점 위치를 manifest에 기록한다. |
| `tank.module.mortar-60mm` | `active`, `damaged`, `disabled` | `88x44` | footprint 중앙에서 발사한다. |
...
1. 2.1단계의 starter 기준 샘플을 먼저 확인한다.
2. `tank.starter.frame.center`를 기준으로 탱크의 광원·외곽선·픽셀 크기를 고정한다.
3. 기준 탱크를 참조해 `machine-gun-12.7mm`과 `mortar-60mm`을 생성한다.
...
- 2.1단계의 palette와 draw box를 변경하지 않고 누락된 항목만 추가한다.

### 2.2.2 샘플 제작

- frame center, 12.7mm 기관총, 60mm 박격포를 먼저 만든다.
- 2x1 모듈의 중심이 양쪽 셀 사이에서 어긋나지 않는지 확인한다.
- damaged와 disabled는 동일한 body를 재사용할 수 있는지 먼저 검토한다.

### 2.2.3 manifest와 provenance 연결

- 모든 탱크 asset에 logical ID, `src`, draw size, pivot, layer, fallback을 등록한다.
- manifest에 없는 파일명을 후속 renderer가 직접 사용하지 않도록 한다.
- `machine-gun-12.7mm`과 `mortar-60mm`의 발사점은 코드의 module world center와 다른 경우 별도 offset으로 기록하되, 기본값은 center로 둔다.

### 2.2.4 Handoff 검증

- 2.3단계가 적 asset을 만들 때 사용할 외곽선 두께와 배경 대비 샘플을 제공한다.
- 2.4단계가 직사·곡사 발사체와 효과를 모듈에 맞춰 제작할 수 있도록 무기 색상과 발사점 규칙을 제공한다.
- 2.6단계가 `Vehicle.render()`와 `CombatModule.render()`를 asset-backed draw로 바꿀 때 필요한 ID와 draw box를 제공한다.
- 2.7단계가 월드 모듈과 혼동되지 않는 UI icon을 선택할 수 있도록 icon 목록을 제공한다.
...
- [x] 빈 cell, blocked cell, 12.7mm 기관총, 60mm 박격포 기준 asset이 있다.
- [x] built-in module과 combat module의 UI icon 목록이 완성되어 있다.
- [x] 1x1, 2x1, 2x2 draw box와 pivot이 2.1단계 계약과 일치한다.
- [x] asset 안에 텍스트, 수치, UI가 없다.

### 시각

- [x] Core와 12.7mm 기관총이 asset preview에서 즉시 구분된다.
...
- [x] 13, 14, 16, 2.7단계가 필요한 logical ID를 문서만 보고 확인할 수 있다.
...
Reference contract: 2.1-art-direction-and-asset-contract.md
Module IDs checked:
World assets:
UI icon assets:
...
- [x] 2.2.1 data audit: `starter`, `blockedCells`, `initialCombatModules`, module `id`·`kind`·`size` 기준을 현재 코드와 대조했다.
- [x] 2.2.2 imagegen: `tank.module.machine-gun-12.7mm`, `tank.module.mortar-60mm`을 생성하고 투명 bounds crop, padding, nearest-neighbor 정규화를 적용했다.
- [x] 2.2.2 deterministic raster: starter frame edge/corner, empty/blocked grid, built-in/combat UI icon 12종을 동일 palette의 투명 PNG로 제작했다.
- [x] 2.2.3 manifest: `src/data/assets.json`에 world asset 7종과 UI icon 12종의 logical ID, runtime path, draw box, pivot, layer, fallback을 등록했다.
- [x] 2.2.3 provenance: 생성 원본, 폐기된 12.7mm 기관총 시도, prompt set, 정규화 규칙, runtime 파일 목록을 `docs/art-asset-provenance.md`에 기록했다.
- [x] 2.2.4 draw box/pivot: 1x1은 44x44, 2x1은 88x44, UI icon은 20x20으로 확인했다. 2x2는 44px cell 두 축을 조합하는 규칙으로 유지한다.
- [x] 2.2.4 transparency: 신규 PNG의 투명 배경과 이미지 내부 텍스트·UI 부재를 확인했다.
- [x] 코드 회귀: `Vehicle`, `CombatGrid`, module size와 renderer 연결 코드는 수정하지 않았다. 실제 asset-backed draw는 2.6단계 handoff로 남겼다.
- [x] 검증: manifest JSON parse, asset path·pixel size 검사, `npx.cmd tsc --noEmit`, `npm.cmd run build`, `git diff --check`를 실행했다.
- [ ] 수동 브라우저 플레이: asset renderer 연결 전 단계이므로 2.6단계에서 fallback과 asset draw를 함께 확인한다.
...
Scope: 12 tank and module art
Reference contract: 2.1-art-direction-and-asset-contract.md
Module IDs checked: core, resource-generator, gatherer, recycler, arsenal, composer, rail, power-pack, caterpillar-track, armor-plate, machine-gun-12.7mm, mortar-60mm
World assets: starter frame edge/corner, empty grid, blocked grid, 12.7mm machine gun, 60mm mortar
