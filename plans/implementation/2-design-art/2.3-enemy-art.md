# 2.3. 적 종류와 적 상태 asset 상세 계획

## 현재 적용 계획

2.1단계의 공통 시각 계약과 2.2단계의 탱크 대비 기준을 사용해 현재 진행 데이터에 정의된 두 적 `standard`, `tanker`의 asset을 제작한다. 새로운 적 AI나 전투 규칙은 추가하지 않고, 기존 상태를 화면에 읽히게 만드는 데 집중한다.

선행 계획:

- [2.1-art-direction-and-asset-contract.md](2.1-art-direction-and-asset-contract.md)
- [2.2-tank-and-module-art.md](2.2-tank-and-module-art.md)
- [1.8-progression-content.md](../1-core-gameplay/1.8-progression-content.md)

후속 계획:

- [2.4-resource-projectile-effects-art.md](2.4-resource-projectile-effects-art.md): 피격·사망·접촉 효과를 적 상태와 연결한다.
- [2.5-map-art.md](2.5-map-art.md): 적이 배경에 묻히지 않는 맵 대비 기준을 사용한다.
- [2.6-asset-loader-canvas-integration.md](2.6-asset-loader-canvas-integration.md): `Enemy.render()`를 asset-backed draw로 연결한다.
- [2.9-art-qa-release.md](2.9-art-qa-release.md): 후반 웨이브의 밀집 상태까지 시각 검수한다.
...
체력 바는 적 body asset에 포함하지 않고 `Enemy.renderHpBar()` 또는 2.6단계 renderer의 overlay로 유지한다.
...
현재 `Enemy`에 hit timer나 contact state가 없다면 2.6단계에서 필요한 최소 상태 값을 추가한다. asset 제작 단계에서 게임 로직을 임의로 추측해 추가하지 않는다.
...
1. 2.1단계 starter 샘플과 2.2단계 탱크 샘플의 광원·외곽선·픽셀 크기를 확인한다.
...
- 탱크 frame과 직사/곡사 발사체 옆에서 두 적의 외곽선이 묻히지 않는지 확인한다.
- 여러 `standard` 사이에 `tanker`를 놓아 silhouette만으로 구분되는지 확인한다.

### 2.3.3 manifest handoff
...
- 2.4단계는 hit/dead/contact 상태의 시간 길이와 effect 위치를 참고한다.
- 2.5단계는 enemy silhouette이 묻히지 않는 배경 대비를 맵마다 검증한다.
- 2.6단계는 `Enemy.render()`에서 asset 선택을 수행하되, `Enemy.update()`와 충돌은 건드리지 않는다.
- 2.9단계는 가장 많은 `standard`가 동시에 존재하는 `core-ruins`를 최종 기준으로 사용한다.
...
- [ ] 적이 탱크 grid 외곽에 도달했을 때 지속 피해 상태가 보인다. 2.6단계 renderer 연결에서 확인한다.
...
- [x] 14, 15, 16, 2.9단계가 필요한 logical ID와 대비 기준을 사용할 수 있다.
...
- [x] 2.3.1 data audit: `progression.json`, `Enemy.ts`, `WaveManager.ts`의 `standard`와 `tanker`, radius 12/18, hp·speed·reward·spawn 규칙을 대조했다.
- [x] 2.3.2 imagegen: 기존 `enemy.standard.idle`를 reference로 사용해 `enemy.tanker.idle`을 생성하고, 차량형으로 나온 첫 시안은 폐기했다.
- [x] 2.3.2 state variants: standard/tanker idle에서 hit/dead를 파생해 같은 중심과 silhouette을 보존했다.
- [x] 2.3.2 support assets: standard/tanker shadow와 grid 접촉 피해용 danger signal을 추가했다.
- [x] 2.3.3 manifest: `src/data/assets.json`에 body 6종, shadow 2종, contact effect 1종을 등록하고 logical draw box, pivot, layer, fallback을 지정했다.
- [x] 2.3.3 provenance: 채택·폐기된 imagegen 원본, prompt set, 정규화 결과, 파생 방식과 runtime 경로를 `docs/art-asset-provenance.md`에 기록했다.
- [x] 2.3.4 회귀 경계: 적 수치, `Enemy.update()`, 접촉 피해 timer, spawn queue, 사망 보상 처리는 수정하지 않았다.
- [x] 검증: manifest JSON parse, 파일 경로·pixel size·투명 모서리 검사, `npx.cmd tsc --noEmit`, `npm.cmd run build`, `git diff --check`를 실행했다.
- [ ] 수동 전투 화면: `Enemy.render()` asset 선택과 contact timer 연결은 2.6단계에서 수행한다.
