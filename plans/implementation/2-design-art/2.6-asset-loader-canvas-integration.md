# 2.6. Asset loader와 Canvas 연결 상세 계획

## 현재 적용 계획

이 단계는 2.1~2.5단계에서 준비한 asset과 manifest를 실제 TypeScript + HTML5 Canvas 렌더링에 연결한다. 게임 규칙, 물리, 충돌, progression 수치는 바꾸지 않고 기존 entity의 도형 renderer를 asset renderer와 fallback renderer로 교체할 수 있게 만든다.
...
- [2.7-hud-upgrade-ui-art.md](2.7-hud-upgrade-ui-art.md)
- [2.8-asset-integration-performance-fallback.md](2.8-asset-integration-performance-fallback.md)
- [2.9-art-qa-release.md](2.9-art-qa-release.md)
...
- `public/assets`와 `src/assets`에는 게임 art 전용 구조가 아직 없으므로 2.1단계 manifest의 경로를 기준으로 추가한다.
...
- `VisualTheme`: 2.1단계에서 확정한 색상과 UI token을 Canvas renderer가 공통으로 사용한다.
...
이 초안은 구현을 강제하는 API가 아니라 2.7~2.9단계가 의존할 안정적인 책임 경계다. manifest 형식과 실제 `Image` cache 타입을 먼저 확정하고 필요한 부분만 구현한다.
...
공통 draw 함수는 world 좌표가 entity의 중심 또는 2.1단계에서 정한 anchor를 의미하도록 한다.
...
- `public/assets/game/`의 2.1~2.5단계 산출물

게임 규칙 파일과 progression 수치는 이 단계에서 수정하지 않는다. 기존 변경이 필요해 보이면 2.9단계 회귀 목록에 기록하고 먼저 원인을 분리한다.
...
- [x] 2.7단계가 사용할 UI icon 로딩 및 draw API가 확인되었다.
- [x] 2.8단계가 재현할 수 있는 load report와 오류 로그가 있다.
- [x] 2.9단계에서 실행할 viewport, region, asset failure 시나리오가 기록되었다.
...
- 별도 `VisualTheme` 파일은 추가하지 않고 SpriteRenderer의 fallback 색상과 기존 HUD token을 작은 범위에서 유지했다. 색상 토큰이 여러 renderer로 확장될 때 2.9단계에서 분리한다.
- 실제 resource collect, armor absorb, Core damage event는 render-only 사건을 만들지 않기 위해 새로 생성하지 않았다. 현재 코드가 발생시키는 direct hit, arc explosion, contact effect만 연결하고 나머지 asset은 2.7~2.9단계의 event/UI/QA handoff로 남겼다.
