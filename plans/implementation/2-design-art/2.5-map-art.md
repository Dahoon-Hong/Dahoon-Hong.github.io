# 2.5. 행성·지역 맵 art 상세 계획

## 현재 적용 계획

이 단계에서는 맵이 전투의 전경을 구성하되, 탱크·적·투사체·자원·HUD보다 시각적 우선순위가 낮은 배경과 장식 asset을 만든다. 2.1단계의 공통 art 계약과 2.2~2.4단계의 전경 asset 대비를 사용한다.
...
- 현재 지역 ID는 다음과 같다.
  ...
  - `cinder/ash-basin`
  - `cinder/core-ruins`
- 전용 맵 JSON이나 맵 asset manifest는 아직 없으므로, 이번 단계에서 새 데이터 계약을 정하고 2.6단계에서 loader에 연결한다.
...
2.1단계의 공통 규칙에 따라 파일 경로는 manifest가 소유하고, 코드에는 논리 asset ID만 전달한다.
...
2.6단계의 Canvas 연결은 다음 순서를 기본값으로 사용한다.
...
실제 스키마는 2.1단계 manifest 계약과 현재 progression 데이터에 맞춰 확정한다.
...
1. 2.1단계의 색상 토큰과 2.2~2.4단계에서 확정된 전경 실루엣을 기준으로 지역별 mood board를 정한다.
...
### 2.5.1 지역 데이터와 기존 progression 비교

- 현재 지역 ID와 표시 이름을 먼저 대조한다.
- 새 asset ID가 progression ID와 일대일로 매칭되는지 확인한다.
- 지역 데이터에 게임 규칙을 복제하지 않고 art 참조만 둔다.

### 2.5.2 배경과 타일 renderer 입력 준비

- background, tile, debris의 논리 크기와 반복 여부를 기록한다.
- HUD 영역을 제외한 전투 영역에만 map layer가 그려지도록 계약한다.
- 2.6단계에서 fallback으로 대체할 수 있는 단순 색상 또는 도형을 함께 정의한다.

### 2.5.3 manifest 연결

- 2.1단계의 manifest에 맵 asset을 추가한다.
- 잘못된 경로, 누락된 region ID, 중복 asset ID를 로딩 단계에서 보고할 수 있도록 한다.

### 2.5.4 다음 단계 인계

- 2.6단계는 `regionId`로 background와 tile을 조회한다.
- 2.8단계는 네 지역을 모두 순회해 asset 누락과 성능을 검사한다.
- 2.9단계는 지역별 스크린샷과 가독성 회귀를 최종 승인한다.
...
- [x] 16, 18, 2.9단계가 사용할 인계 정보를 이 문서에 기록했다.
...
- 2.5단계 map ID와 runtime path, draw box, pivot, layer, fallback은 `src/data/assets.json`, 생성 source와 정규화 내용은 `docs/art-asset-provenance.md`에 기록했다.

### 완료 범위와 handoff

- 네 지역의 art asset과 데이터 계약은 완료했다.
- 실제 foreground와의 overlap, combat effect 대비, HUD 패널 clipping은 map renderer가 연결되는 2.6단계와 최종 art QA 2.9단계에서 확인한다.
