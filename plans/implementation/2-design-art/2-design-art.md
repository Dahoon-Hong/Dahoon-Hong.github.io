...
이 문서는 와이어프레임 단계의 Canvas 도형 렌더링을 최종 목표인 2D 픽셀 아트와 게임 UI로 확장하기 위한 상위 실행 계획이다. 2.1~2.9 단계는 한 번에 모든 이미지를 교체하지 않고, 공통 규격을 먼저 고정한 뒤 탱크·적·자원·맵·UI를 순서대로 제작하고 마지막에 코드 연결과 시각 QA를 수행한다.

기능 규칙과 콘텐츠 수치는 기존 계획을 따른다.

- 모듈·전투 그리드 구조: [1.7-mid-term-review.md](../1-core-gameplay/1.7-mid-term-review.md)
- 탱크·적·행성·지역 콘텐츠: [1.8-progression-content.md](../1-core-gameplay/1.8-progression-content.md)
- 업그레이드 웹과 튜토리얼 UI: [99-ui-tutorial.md](../99-ui-tutorial/99-ui-tutorial.md)
- 배포 전 검증: [100-release-verification.md](../100-release-verification/100-release-verification.md)
- 전체 기준선: [1-core-gameplay.md](../1-core-gameplay/1-core-gameplay.md)
...
2.1단계에서 아래 계약을 먼저 확정하고 2.2~2.9 단계에서 변경하지 않는다.
...
2.2~2.5는 2.1의 규격이 확정되면 병렬 제작할 수 있지만, 2.6의 코드 연결은 최소 한 종류의 기준 asset과 manifest가 준비된 뒤 시작한다. 2.7은 2.6에서 확정한 색·타이포그래피·아이콘 규칙을 사용한다.

## 2.1. 아트 디렉션과 공통 asset 규격

상세 실행 계획: [2.1-art-direction-and-asset-contract.md](2.1-art-direction-and-asset-contract.md)
...
- 전투 모듈: `machine-gun-12.7mm`, `mortar-60mm` 및 이후 탱크별 전투 모듈
...
## 2.8. 전체 asset 통합·성능·폴백 검증

### 목표

2.2~2.7에서 만든 asset과 renderer를 전체 게임 루프에 연결하고, 생성 이미지가 실제 플레이 성능과 안정성을 해치지 않도록 정리한다.
...
- 기존 1.1~1.8 및 99~100의 기능·튜토리얼·배포 완료 조건이 깨지지 않는다.
...
- [2.1-art-direction-and-asset-contract.md](2.1-art-direction-and-asset-contract.md): 공통 art direction, logical ID, manifest, fallback 계약
- [2.2-tank-and-module-art.md](2.2-tank-and-module-art.md): 탱크·Core·내장 시스템·전투 모듈 asset
- [2.3-enemy-art.md](2.3-enemy-art.md): standard/tanker와 피해·접촉·파괴 상태 asset
- [2.4-resource-projectile-effects-art.md](2.4-resource-projectile-effects-art.md): 자원·투사체·피격·폭발 효과 asset
- [2.5-map-art.md](2.5-map-art.md): 행성·지역 배경, 타일, 장식 asset
- [2.6-asset-loader-canvas-integration.md](2.6-asset-loader-canvas-integration.md): loader, cache, Canvas sprite/fallback 연결
- [2.7-hud-upgrade-ui-art.md](2.7-hud-upgrade-ui-art.md): HUD, 설치, upgrade web, pause/result UI art
- [2.8-asset-integration-performance-fallback.md](2.8-asset-integration-performance-fallback.md): 전체 asset coverage, 성능, 오류와 fallback 통합 검증
- [2.9-art-qa-release.md](2.9-art-qa-release.md): 시각·기능 QA, release candidate, 최종 승인
