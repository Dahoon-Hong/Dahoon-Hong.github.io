# 2.4. 자원·발사체·피격·폭발 효과 asset 상세 계획

## 현재 적용 계획

2.1단계의 공통 asset 계약과 12·2.3단계의 actor 시각 기준을 사용해 자원 흐름과 전투 사건을 제작한다. 이 단계는 플레이어가 무엇을 얻었고, 어떤 무기가 적중했으며, 장갑 또는 Core에 어떤 피해가 전달됐는지를 짧은 피드백으로 알아보게 만드는 단계다.
...
- [1.4-resource-collection.md](../1-core-gameplay/1.4-resource-collection.md)
- [1.5-production-and-logistics.md](../1-core-gameplay/1.5-production-and-logistics.md)

후속 계획:

- [2.5-map-art.md](2.5-map-art.md): 효과가 잘 보이는 배경 명도 기준을 공유한다.
- [2.6-asset-loader-canvas-integration.md](2.6-asset-loader-canvas-integration.md): projectile, pickup, effect draw를 연결한다.
- [2.7-hud-upgrade-ui-art.md](2.7-hud-upgrade-ui-art.md): world feedback와 HUD 숫자·icon을 일치시킨다.
- [2.8-asset-integration-performance-fallback.md](2.8-asset-integration-performance-fallback.md): 효과 누적 성능과 일시정지 동작을 검증한다.
...
- 발사점은 2.2단계의 12.7mm 기관총 중심과 일치한다.
...
1. 2.2단계의 기관총/박격포 모듈과 2.3단계의 enemy silhouette을 기준으로 effect 색과 크기를 맞춘다.
...
- 2.5단계는 배경 명도를 effect가 읽히는 수준으로 유지한다.
- 2.6단계는 update 코드와 render asset을 연결하며, 실제 사건 발생 위치에서만 effect를 생성한다.
- 2.7단계는 resource icon과 world pickup 의미를 맞춘다.
- 2.8단계는 많은 적과 동시 폭발에서 effect 상한과 preload를 검증한다.
...
- [x] 2.5단계가 effect 대비를 고려한 배경을 만들 수 있다.
- [ ] 2.6단계가 각 effect의 생성 위치와 fallback을 연결할 수 있다.
- [ ] 2.7단계가 resource icon과 HUD 수치를 같은 의미로 사용할 수 있다.
- [ ] 2.8단계가 effect 개수 상한과 성능 검증을 수행할 수 있다.
...
- pause 중 자동 생산과 자원 수집이 멈추는 기존 규칙은 유지하고, 2.4단계에서는 gameplay event나 수치 계산을 변경하지 않았다.
...
- 2.4단계 asset 및 visual contract는 완료했다.
- 실제 hit 위치, 사망과 reward pickup 위치, AOE radius transform, pause 중 effect 생성 차단, reduced-motion 동작은 renderer가 연결되는 2.6단계와 성능·접근성 검증 2.8단계에서 확인한다.
- 효과 개수 상한과 preload는 2.8단계에서 결정하며, 이 단계에서는 무한 particle loop나 camera shake를 추가하지 않았다.
