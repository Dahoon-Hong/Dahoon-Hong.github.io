# 2.7. HUD·upgrade UI art 상세 계획

## 현재 적용 계획

2.6단계에서 확정한 asset loader와 Canvas renderer를 사용해 HUD, 모듈 설치, upgrade web, pause, 결과 화면의 시각 체계를 정리한다. 기존 UI의 좌표, hitbox, 텍스트 의미, 게임 규칙은 유지하고 visual hierarchy와 상태 표현을 개선한다.
...
- [99-ui-tutorial.md](../99-ui-tutorial/99-ui-tutorial.md)
...
- `99-ui-tutorial.md`는 현재 기능과 tutorial 문구의 기준이며, 이 단계는 문구를 임의로 다시 쓰지 않는다.
...
- 2.1단계의 pixel scale과 `imageSmoothingEnabled = false`를 UI icon에도 적용한다.
...
2.1단계 manifest에 다음 logical ID를 등록한다.
...
### 2.7.1 현재 UI inventory 고정

- `HUDManager`의 모든 draw section, label, hitbox, 상태 분기를 표로 만든다.
- `99-ui-tutorial.md`의 문구와 현재 조작 흐름을 대조한다.
...
### 2.7.3 icon과 frame asset 연결

- 2.6단계의 `renderer.drawSprite()`로 icon을 그린다.
- icon이 누락되면 해당 상태를 암시하는 단순 도형 fallback을 사용한다.
- frame, pivot, alpha가 click rect와 별개로 동작하는지 확인한다.

### 2.7.4 Canvas UI component 정리
...
- [x] 2.6단계 loader의 missing/failed fallback이 UI에서도 동작한다.
- [x] 2.8단계가 검사할 UI 상태와 viewport 목록이 기록되었다.
- [x] 2.9단계에서 99, 100과 함께 기능 회귀를 재현할 수 있다.
...
- `HUDManager`가 2.6단계 `RenderContext`를 받아 기존 manifest의 Core·자원·모듈 icon을 그리며, asset이 없거나 로딩에 실패하면 `SpriteRenderer` fallback 경로를 유지한다.
...
- 새 raster UI 파일은 추가하지 않았다. 현재 manifest의 icon과 2.6단계 fallback, Canvas shape marker로 pause/wave/status 의미를 표현해 manifest에 존재하지 않는 빈 asset 경로를 만들지 않았다.
...
viewport와 DPR: logical 1280x720, 기존 getBoundingClientRect() 입력 변환 유지; 고해상도 실브라우저 캡처는 2.9단계 QA에서 확인
