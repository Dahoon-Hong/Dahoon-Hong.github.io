# 2.8. Asset 통합·성능·fallback 상세 계획

## 현재 적용 계획

이 단계에서는 2.1~2.7단계의 asset, renderer, 맵, HUD를 하나의 실행 흐름으로 검증한다. 새로운 art style이나 gameplay를 추가하지 않고, 누락 asset과 로딩 지연이 있어도 플레이 가능한 상태를 유지하는 데 집중한다.
...
- 2.9단계 release QA가 재현할 수 있는 검사 결과와 오류 목록을 남긴다.
...
| --- | --- | --- | --- | --- |
| tank | scout, striker, bulwark와 module IDs | 2.2단계 산출물 | tank silhouette | 미확인 |
| enemy | standard, tanker와 상태 frame | 2.3단계 산출물 | enemy shape | 미확인 |
| resource | pickup과 matter/ammo/nano icon | 2.4단계 산출물 | diamond/icon shape | 미확인 |
| map | 네 region background/tile/debris | 2.5단계 산출물 | solid field | 미확인 |
| projectile/effect | direct, arc, impact, contact, death | 2.4단계 산출물 | line/ring | 미확인 |
| UI | core, resource, state, upgrade icons | 2.7단계 산출물 | geometric icon | 미확인 |
...
### 2.8.1 manifest와 파일 audit

- 2.1~2.7단계의 asset ID를 하나의 목록으로 합친다.
- 각 ID의 실제 파일, draw size, pivot, frame, fallback, 사용처를 확인한다.
- 누락은 종류별로 기록하고 2.9단계에서 release blocker인지 결정한다.

### 2.8.2 loader와 cache audit
...
- 최종 load report와 asset coverage 표를 2.9단계 입력으로 남긴다.
...
- [ ] 2.9단계가 사용할 blocker, warning, fallback 목록이 남아 있다.
