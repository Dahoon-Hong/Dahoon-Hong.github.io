# 2.9. Art QA와 release 승인 상세 계획

## 현재 적용 계획

이 단계는 2.1~2.8단계에서 만든 art direction, asset, Canvas 연결, HUD, fallback을 최종 release gate로 검증한다. 이 단계에서 새 gameplay나 새로운 시각 방향을 추가하지 않는다. 결함이 발견되면 원래 책임 단계로 되돌려 수정하고, 다시 이 문서의 해당 검사를 실행한다.
...
### 2.9.1 release candidate 고정

- 2.8단계의 asset coverage, load report, 성능 결과를 기준선으로 복사한다.
...
### 2.9.4 asset failure와 fallback 실행
