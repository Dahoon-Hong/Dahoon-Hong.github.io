# 2.1. 아트 디렉션과 공통 asset 규격 상세 계획

## 현재 적용 계획

이 단계는 2번 로드맵의 첫 실행 단계다. 와이어프레임 도형을 최종 픽셀 아트로 교체하기 전에, 모든 asset이 따라야 할 시점·팔레트·크기·pivot·명명·상태·폴백 규칙을 고정한다.

이 단계의 결과물은 2.2~2.9에서 반복해서 참조하는 아트 계약이다. 따라서 2.1단계가 끝나기 전에는 대량의 탱크·적·맵 asset을 만들거나 renderer 코드를 연결하지 않는다.

상위 계획: [2-design-art.md](2-design-art.md)

후속 계획:

- [2.2-tank-and-module-art.md](2.2-tank-and-module-art.md)
- [2.3-enemy-art.md](2.3-enemy-art.md)
- [2.4-resource-projectile-effects-art.md](2.4-resource-projectile-effects-art.md)
- [2.5-map-art.md](2.5-map-art.md)
- [2.6-asset-loader-canvas-integration.md](2.6-asset-loader-canvas-integration.md)

선행 기준:

- 모듈과 전투 그리드: [1.7-mid-term-review.md](../1-core-gameplay/1.7-mid-term-review.md)
- 적·행성·지역 콘텐츠: [1.8-progression-content.md](../1-core-gameplay/1.8-progression-content.md)
...
2.1단계 구현 시작 시 아래 기준을 실제 코드에서 다시 확인하고, 변경점이 있으면 이 문서의 기준표를 먼저 갱신한다.

| 표면 | 현재 기준 | 2.1단계에서 지킬 계약 |
|---|---|---|
...
| HUD | Canvas에서 상단 바와 오른쪽 패널을 렌더링 | 2.7단계 전까지 Canvas 좌표와 click hitbox 계약을 유지한다. |

### 현재 색상 감사

현재 코드에 흩어진 값을 아래 semantic token으로 정리한다. 2.1단계에서는 시각 계약을 문서화하고, 실제 코드의 상수 통합은 renderer 연결 단계에서 수행한다.
...
`#000000`은 현재 일부 글자와 결과 버튼에 사용되는 legacy 값이다. 최종 시각 구현에서는 배경과 충돌하지 않는 어두운 잉크 token으로 교체할 수 있으나, 2.1단계에서 임의의 새 색을 추가하지 않는다.
...
- `imageSmoothingEnabled = false`는 2.6단계 renderer 연결 때 적용한다. 2.1단계에서는 모든 샘플을 이 규칙으로 검수한다.
...
이렇게 하면 grid 크기와 Core Engine system을 분리한 채 탱크 frame을 여러 grid 크기에 재사용할 수 있다. 조립 규칙은 2.2단계에서 탱크 asset을 제작할 때 사용하고, 실제 조합 renderer는 2.6단계에서 구현한다.
...
tank.module.machine-gun-12.7mm.active
tank.module.mortar-60mm.active
enemy.standard.idle
enemy.tanker.hit
...
public/assets/game/
  tank/
    ...
    module-machine-gun-12.7mm-active.png
    module-mortar-60mm-active.png
  enemies/
    standard-idle.png
    standard-hit.png
    tanker-idle.png
...
2.1단계에서 전체 파일을 구현할 필요는 없지만, 2.2단계의 첫 asset부터 아래 형식을 따른다. 실제 loader 타입은 2.6단계에서 추가한다.
...
추가 필드가 필요하면 2.1단계 art bible과 manifest 예제를 먼저 갱신한다. renderer가 파일명 규칙이나 이미지 원본 크기를 추측하지 않게 한다.
...
- Core, 12.7mm 기관총, 60mm 박격포, armor plate가 서로 다른 기능을 암시한다.
...
실제 이미지가 필요한 2.2~2.5단계에서는 `imagegen`을 사용해 기준 asset부터 만든다. 한 번에 모든 변형을 만들지 않고, 기준 샘플의 구도·팔레트·광원·투명 여백을 확인한 뒤 파생 asset을 만든다.

2.1단계에서 생성할 기준 샘플은 다음 4종이다.
...
이 샘플은 최종 콘텐츠 전량이 아니다. 2.2~2.7에서 같은 스타일을 유지하기 위한 visual anchor다.
...
2.1단계 구현이 끝났을 때 다음 항목이 존재해야 한다.
...
- [ ] 기존 1.1~1.8 기능과 99~100 선행 조건을 변경하지 않았다.
- [ ] 일시정지 중 모듈 설치·업그레이드만 가능하고 자동 생산·수집·변환은 계속 멈춘다.

## 다음 단계로 넘길 결정

2.1단계가 완료되면 아래 결정은 다시 논의하지 않고 2.2~2.9에서 사용한다.
...
검증 기록에 실패 원인을 남기고, asset 계약을 바꾼 경우에는 2.2~2.9 단계의 선행 조건도 함께 갱신한다.
