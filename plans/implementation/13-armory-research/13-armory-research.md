...
- 연구 후 `arc-weapon`(표시명 Mortar Grenade)의 구매 가능 상태와 기존 비용·재고 흐름은 유지한다.
...
1. 재생 상태에서 Armory의 `research-arc-weapon` 노드를 선택할 수 있다.
2. 연구 성공 후 Mortar Grenade 구매 버튼이 활성화된다.
...
- 기존 `arc-weapon` 연구 연결과 구매 비용·재고 처리는 변경하지 않았다.
