...
- 처치 수·경과 시간 등을 합산하는 별도 위협값은 이번 구현에 넣지 않는다. 후속 위협수치 시스템은 Plan 14에서 맵 기본값·선택 난이도·Stage 경과 시간 기반으로 같은 스폰 정책에 연결한다.
...
## 후속 plan 연결: ThreatMeter

처치 수·경과 시간 등을 합산하는 후속 위협수치 시스템은 backlog에 남겨 두지 않고 [Plan 14: 위협수치 기반 난이도 시스템](../../../plans/implementation/14-threat-based-difficulty/14-threat-based-difficulty.md)으로 승격한다.

Plan 14는 `currentWave`만 사용하는 현재 경계를 대체하고, 맵 기본값·선택 난이도·Stage 경과 시간으로 위협 배율을 계산한다.

Plan 14의 첫 구현은 처치 수·생존 적 수를 합산하지 않고 Stage 경과 시간만 사용한다. 이후 다른 신호를 추가할 때는 별도 설계와 밸런스 검증으로 범위를 확장한다.
