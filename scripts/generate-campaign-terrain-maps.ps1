...
function Get-RegionPolygons($map) {
  ...
  for ($y = 0; $y -lt $map.terrain.rows.Count; $y++) {
    ...
    while ($x -lt $row.Length) {
      if ($row[$x] -ne 'H') { $x++; continue }
      $start = $x
      while ($x -lt $row.Length -and $row[$x] -eq 'H') { $x++ }
  ...
  return @($rectangles | ForEach-Object {
    ,@(
      [System.Drawing.Point]::new($_.x * 36, $_.y * 36),
      [System.Drawing.Point]::new(($_.x + $_.width) * 36, $_.y * 36),
      [System.Drawing.Point]::new(($_.x + $_.width) * 36, ($_.y + $_.height) * 36),
      [System.Drawing.Point]::new($_.x * 36, ($_.y + $_.height) * 36)
    )
  })
}
